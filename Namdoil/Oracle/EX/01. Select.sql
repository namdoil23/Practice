...
-- 4. 연결연산자(||)
-- 한 번에 연결해서 조회하고 싶은 컬럼들을 ||을 이용해서 연결 조회할 수 있따.
...
-- 5-2. DISTINCT 사용해서 중복제거 했을 떄
SELECT DISTINCT JOB
	 FROM EMP; 
	 
-- 5-3. 컬럼 여러개에 대한 DISTINCT
-- 여러 개의 컬럼을 조회할 때 DISTINCT
-- 조회된 여러개의 컬럼을 데이터 셋 하나로 인식해서
-- 조회된 모든 컬럼에 데이터가 중복돼야 중복으로 인식한다.
SELECT DISTINCT JOB
	 , MGR
	 FROM EMP e 
...
-- 6-1. 컬럼 하나만 정렬
-- 오름차순(ASC)로 정렬할 떄는 ASC를 생략해도 된다.
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 내림차순(DSEC)로 정렬할 때는 항상 DESC를 명시해야 한다.
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 6-2. 컬럼 여러개로 정렬
-- 처음 지정된 컬럼으로 정렬된 상태에서 다음 지정된 컬럼으로 정렬한다.
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 사원중에 급여가 300이상인 사원번호, 사원이름, 급여 조회
SELECT ENO
	 , ENAME
	 , SAL
	 FROM EMP e 
...
-- 학생중에 평점이 3.0이하인 학생번호, 학생이름, 전공, 학년, 평점조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 전공이 화학과가 아닌 학생의 학생번호, 학생이름, 전공, 학년조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 급여컬럼에 있는 모든 데이터가 문자열로 변화되는 과정이 필요하다.
...
-- 과목 중에 화학이 포함되어 있는 과목번호, 과목이름, 학점 조회
SELECT CNO
	 , CNAME
	 , ST_NUM 
	 FROM COURSE c 
...
-- 사원중에 회계업무를 하면서 급여가 3000이상인 이름이 세글자인 사원의 사원번호, 사원이름, 업무, 급여 조회
SELECT ENO
	 , ENAME
	 , JOB
	 , SAL
...
-- () 조건의 우선순위를 정해준다.
-- 학생 중 전공이 화학이거나 물리인 학생중 1학년인 학생의 학생번호, 학생이름, 전공, 학년 조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 급여가 300이상 500이하인 사원의 사원번호, 사원이름, 급여 조회
SELECT ENO
	 , ENAME
	 , SAL
	 FROM EMP e 
...
-- ALTER SESSION SET NLS_DATE_FORMAT = 'yyyy/MM/dd HH24:mi:ss'; (오라클에서만 됨)


-- BETWEEN AND 구문에서 두 날짜 사이의 있는 데이터도 조회할 수 있다.
-- TO_DATE: 데이터의 형식을 날짜형식으로 변경한다.
-- TO_DATE(문자열, 문자열의날짜형식) 
...
SELECT PNO
	 ...
	 WHERE HIREDATE 
	 	BETWEEN TO_DATE('20000101 00:00:00', 'yyyyMMdd HH24:mi:ss')
	 		AND TO_DATE('20011231 23:59:59', 'yyyyMMdd HH24:mi:ss') 
	 
-- 10. 여러개의 데이터와 값을 비교하는 IN절
-- 전공이 화학이거나 물리인 학생의 학생번호, 학생이름, 전공, 학년 조회
SELECT SNO
	 , SNAME
	 , MAJOR
...
-- 거의 모든 DBMS에서 문자열은 작은 따옴표('')로 표시된다.
