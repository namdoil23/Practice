...
-- ROLLUP 함수를 이용해서 전공별 평균평점, 평점의 총합, 최고 평점 조회
...
-- ROLLUP 함수가 GROUP BY에 지정된 컬럼을 뒤에서부터 하나씩 그룹화를 진행한다면
...
-- PIVOT은 FROM절에서 그룹화할 칼럼, 통계낼 컬럼만 조회하는 서브쿼리를 사용해야된다.
SELECT *
	FROM EMP 
	PIVOT(
	 	MAX(SAL)
...
-- PIVOT으로 생성한 결과는 SELECT 문에서 바로 사용가능	
SELECT OPER
	 , 지원
	 , ACCOUNT
	 , DEVELOP
...
-- 전공별 학년별 평점의 최고 평점 조회하는데 PIVOT을 이용해서 전공을 컬럼으로 조회	
SELECT *
	 FROM (
	 	   SELECT MAJOR
	 			, SYEAR
