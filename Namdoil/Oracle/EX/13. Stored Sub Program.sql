...
-- ST_SC_GRADE 테이블에 학생별 과목별 기말고사 성적 등급까지 저장하는 프로시저 P_ST_SC_GRADE 생성
...
-- 1-3. Stored Function: 오라클 사용자 정의 함수. 프로시저와 동일한 역활을 하지만
...
CREATE OR REPLACE FUNCTION F_GET_AVR
(
    AVR NUMBER
)
-- 함수의 결과의 데이터타입 지정
...
SELECT F_GET_AVR(AVR) FROM STUDENT;

-- 날짜를 파라미터로 받아서 10년뒤 날짜를 리턴하는 함수 F_PLUS_10YESRS를 만드세요.
CREATE OR REPLACE FUNCTION F_PLUS_10YESRS
(
     TENYEAR DATE
)
-- 함수의 결과의 데이터타입 지정
...
BEGIN    
    ...
    -- 위에서 지정한 함수 결과의 데이터 타입의 값을 RETURN 구문을 사용해서 RETURN
    RETURN ADD_MONTHS(TENYEAR, 120);
END;
/

SELECT F_PLUS_10YESRS(HDATE) FROM EMP;
...
BEGIN    
    IF HDATE1 >= HDATE2 THEN DATEDIFF := HDATE1 - HDATE2;
    ELSE DATEDIFF := HDATE2 - HDATE1;
    END IF;
...
SELECT ENAME
     , HDATE
     , F_DIFF_DATES(SYSDATE, HDATE)"입사후 지난날짜" 
     FROM EMP; 
...
-- EMP 테이블의 SAL 컬럼의 값이 변경되거나 추가되기 전에 동작하는 트리커
BEFORE 
...
BEGIN 
    ...
    WHEN NOT MATCHED THEN
        ...
        VALUES(:NEW.SNO, ST_SC_GRADE_ROW.SNAME, 
        :NEW.CNO, ST_SC_GRADE_ROW.CNAME, :NEW.RESULT, 
        ST_SC_GRADE_ROW.GRADE); 
END;
/
