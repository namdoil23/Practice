...
-- 교수가 정교수인 부교수인지 출력하는 PL/SQL을 작성하세요
...
BEGIN
	LOOP
		-- 조회하는 데이터의 행의 개수가 0이면 NO_DATA_FOUND 예외가 발생한다.
...
DECLARE 
	...
	TYPE STUDENT_REC IS RECORD(
		-- 기본 데이터 타입과 참조형 데이터타입들을 모두 사용할 수 있다.
...
-- 7-2. 연관 배열을 ROWYPE으로 생성해서 
-- STUDENT_RECORD 테이블에 SNO 20001~20009번까지의 학생을 저장하세요.
-- SNO, SNAME, SEX, SYEAR, MAJOR, AVR은 위의 내용 참고

DECLARE
	TYPE STU_REC_ARR_ROW IS TABLE OF STUDENT_RECORD%ROWTYPE
	INDEX BY PLS_INTEGER;
	
	STURECARRROW STU_REC_ARR_ROW;
	IDX NUMBER := 1;	
	
BEGIN
	LOOP
		SELECT DECODE(MOD(IDX, 2), 0, '남', '여') INTO STURECARRROW(IDX). SEX
			FROM DUAL;
		
		STURECARRROW(IDX).SNO := TO_CHAR(20000 + IDX);
		STURECARRROW(IDX).SNAME := '학생'||(20000 + IDX);
		STURECARRROW(IDX).SYEAR := MOD(IDX, 4) + 1;
		STURECARRROW(IDX).MAJOR := '컴공';
		STURECARRROW(IDX).AVR	:= MOD(IDX, 4) + 0.24;
	
	INSERT INTO STUDENT_RECORD 
		VALUES STURECARRROW(IDX);
		COMMIT;
...
BEGIN 
	...
	LOOP 
		...
		DBMS_OUTPUT.PUT_LINE('1학년 학생 중 평점이 2.7인 학생목록');
		DBMS_OUTPUT.PUT_LINE(STUREC.SNO);
		DBMS_OUTPUT.PUT_LINE(STUREC.SNAME);
		DBMS_OUTPUT.PUT_LINE(STUREC.SYEAR);
		DBMS_OUTPUT.PUT_LINE(STUREC.MAJOR);
	...
	LOOP 
		...
		DBMS_OUTPUT.PUT_LINE('3학년 학생 중 평점이 1.5인 학생목록');
		DBMS_OUTPUT.PUT_LINE(STUREC.SNO);
		DBMS_OUTPUT.PUT_LINE(STUREC.SNAME);
		DBMS_OUTPUT.PUT_LINE(STUREC.SYEAR);
		DBMS_OUTPUT.PUT_LINE(STUREC.MAJOR);
...
--	EMPRECROW EMP_REC_ROW;
	EMPRECROW EMP%ROWTYPE;

BEGIN 
	FOR EMPRECROW IN EMP_CUR('01',3000) LOOP
		DBMS_OUTPUT.PUT_LINE(EMPRECROW.DNO);
		DBMS_OUTPUT.PUT_LINE(EMPRECROW.SAL);
		DBMS_OUTPUT.PUT_LINE(EMPRECROW.ENAME);
		DBMS_OUTPUT.PUT_LINE('-------------------');
	END LOOP;
...
-- 7. 예외처리: EXCPTION 부에서 PL/SQL이 동작하다가 발생할 수 있는 예외에 대한 처리를 작성한다.
DECLARE
	VAL_SNO NUMBER;
BEGIN
