...
-- 첫 번째 쿼리에서 컬럼의 개수, 데이터 타입이 결정되기 때문에 두번째 쿼리는 첫 번째 쿼리의 컬럼의 개수, 데이터 타입을 따라야한다.
SELECT PNO
	 , PNAME
	 , HIREDATE
	 FROM PROFESSOR
...
-- 평점이 3.0이상인 학생의 학생번호, 학생이름, 학년, 평점과 학년이 3학년인 학생번호, 학생이름, 학년, 평점을 함께 조회
SELECT SNO
	 , SNAME
	 , SYEAR
	 , AVR
...
INSERT INTO EMP VALUES('9999', '제갈공', '지원', NULL, SYSDATE, 3200, 320, NULL);
COMMIT;
...
-- 교집합 연산자를 사용해서 물리, 화학과 학생 중 평점 3.0이상인 학생의 학생번호, 학생이름, 전공, 평점 조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , AVR 
