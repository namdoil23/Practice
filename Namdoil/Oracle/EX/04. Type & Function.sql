...
-- TO_DATE(문자열 데이터, 문자열 데이터의 날짜 형식):매개변수로 지정된 날짜형식으로 되어있는 문자열을 DATE 타입으로 변화 후 리턴
...
-- ROUND(실수), TRUNC(실수): 정수까지(소수점 첫째자리에서), 정수까지(소수점 첫째자리에서) 버림
SELECT ROUND(1.45, 0)
	 , ROUND(1.45)
	 FROM DUAL;
	
-- 1-8. NVL: NULL 처리 함수
-- 과목의 과목번호, 과목이름, 교수번호, 교수이름을 조회하는데 담당교수가 배정되지 않은 과목과 과목을 배정받지 못한 교수의 정보도 함계 조회
-- 담당교수가 NULL인 데이터는 교수가 배정되지 않음이라고 조회하고
...
--				컬럼의 데이타나 조회해온 데이터가 조건 값1과 일치할때 실행될내용,
--				컬럼의 데이타나 조회해온 데이터가 조건 값1과 일치하지 않을때 실행될내용)
-- DECODE는 조건문을 처리하는 구문으로 조건에는 값만 넣어서 사용할 수 있다.
SELECT ENO
	 , ENAME
	 , DNO
	 , DECODE(DNO, '10', '인사팀', '다른부서') AS"부서명"
	 FROM EMP
	 ORDER BY DNO; 
			 
-- 1-9. DECODE(컬럼명이나 조회해온 데이터, 조건 값1, 
--				컬럼의 데이타나 조회해온 데이터가 조건 값1과 일치할때 실행될 내용,
--				조건 값2,
--				컬럼의 데이타나 조회해온 데이터가 조건 값2과 일치할때 실행될 내용,
--				조건 값3,
--				컬럼의 데이타나 조회해온 데이터가 조건 값3과 일치할때 실행될 내용,
--				...
--				조건 값N,
--				컬럼의 데이터나 조회해온 데이터가 조건 값N과 일치할때 실행될 내용,
--				컬럼의 데이터나 조회해온 데이터가 조건 값N과 일치하지 않을때 실행될 내용(위의 조건이 모두 일치하지 않을 경우))	
...
-- 업무가 개발이면 50%인상, 업무가 경영이면 30% 인상, 지원이면 20%인상 그외 업무면 10%인상 된 인상급여 조회
SELECT ENO
	 , ENAME
	 , JOB
	 , SAL
...
-- 		WHEN 조건2
-- 		THEN 조건2이 TRUE 일때 처리할 내용		
--		...	
-- 		WHEN 조건N
-- 		THEN 조건N이 TRUE 일때 처리할 내용		
-- 		ELSE
-- 		위 조건이 모두 FALSE인 데이터에 대한 처리내용
-- 		END AS 별칭
-- CASE 절에 컬럼이나 데이터를 지정한 경우(SWITCH CASE 문처럼 사용)
SELECT ENO
	 , ENAME
	 , COMM
	 , CASE NVL(COMM, -1)
