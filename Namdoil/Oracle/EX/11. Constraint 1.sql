...
--							제약 조건에는 PK, FK, UK, NOT NULL, DEFAULT, CHECK 등이 존재한다.
-- 1-1. PK(PRIMARY KEY): 테이블에 식별자 역활을 하는 컬럼을 지정
--							PK로 지정된 컬럼의 데이터는 유일해야 하면(UK), 색인 역활도 하면(INDEX), 
...
-- 다중 컬럼으로 PK를 지정하게 되면 PK로 지정된 칼럼들을 데이터 셋으로 묶어서 PK로 인식한다.
-- PK로 지정된 칼럼들은 데이터 모두 중복돼야 중복으로 인식한다.
...
-- PK로 지정된 컬럼의 데이터에 중복값이나 NULL값이 없어야 한다.
...
-- 1-2. FOREIGIN KEY(외래키, 참조키): 다른 테이블의 PK나 UK로 설정된 컬럼을 참조하여 제약조건을 생성
-- 									참조한 컬럼에 존재하는 데이터만 저장할 수 있는 제약조건이 생성된다.
-- 									테이블간의 관계를 맺어주는 제약조건
--									참조하는 테이블이 부모 테이블이 되고 참조해오는 테이블이 자식 테이블이 돼서
--									테이블간의 종속관계를 형성한다. 부모테이블에 존재하는 데이터만 저장가능
-- 									관계는 1:1, 1:N, N:1, N:N 등 다양한 형태가 존재한다.
-- DEPT_PK1 테이블의 PK로 설정된 DNO을 참조한 FK를 갖는 EMP_KF1 테이블 생성
...
-- 식별자 역활은 하지 않아서 중복된 값을 여러개 지정할 수 있다.
INSERT INTO EMP_FK1
VALUES (3, '장길산', 1);
COMMIT;
...
-- 컬럼을 모두 지정하고 FK 생성
-- CASCADE 옵션 추가하고 생성, ORACLE에서는 DELETE CASCADE 옵션만 추가 가능
-- CASCADE 옵션은 추가하면 부모테이블 데이터의 수정이나 삭제가 일어날 경우
...
-- FK면서 식별자 역활을 하기 때문에 데이터의 중복이 허용되지 않고 NULL 값도 저장할 수 없다.
INSERT INTO SCORE_PK1_FK1
VALUES('1214', '111111', 100);
COMMIT;

INSERT INTO SCORE_PK1_FK1
VALUES('1214', '222222', 100);
...
-- SNO, CNO의 묶음을 PK로 지정하는 테이블 ST_SCORE_PK_FK를 생성하세요(SNO, CNO)
ALTER TABLE STUDENT ADD CONSTRAINT PK_STUDENT_SNO PRIMARY KEY(SNO);

CREATE TABLE ST_SCORE_PK_FK(
	SNO VARCHAR2(8),
	CNO VARCHAR2(8),
	CONSTRAINT FK_ST_SCORE_PK_FK_SNO FOREIGN KEY(SNO)
	REFERENCES STUDENT(SNO),
	CONSTRAINT FK_ST_SCORE_PK_FK_CNO FOREIGN KEY(CNO)
	REFERENCES COURSE(CNO),
	CONSTRAINT PK_ST_SCORE_PK_FK_SNO_CNO PRIMARY KEY(SNO, CNO)
);
...
-- 1대 1 관계
-- 부모테이블에 존재하는 테이터가 자식테이블에도 하나만 존재할 수 있는 구조
CREATE TABLE MEMBER(
	ID NUMBER(9),
...
CREATE TABLE BOARD_FILE_FK(
	...
	CONSTRAINT FK_BOARD_FILE_FK_FILE_ID FOREIGN KEY(BOARD_ID)
...
SELECT B.BOARD_ID
	 ...
	 JOIN BOARD_FILE_FK BF
	   ON B.BOARD_ID = B.BOARD_ID;

-- N대 N관계(다대다 관계): 두 개의 테이블의 테이터가 여러개 존재하는 N대N관계가 매핑된 구조
...
-- LECTURE 테이블과 STUD 테이블이 N대 N 관계로 매핑된 테이블
...
INSERT INTO LECTURE VALUES(1, '자바');
INSERT INTO LECTURE VALUES(2, '오라클');
COMMIT;

INSERT INTO STUD VALUES(1, '고기천');
INSERT INTO STUD VALUES(2, '홍길동');
INSERT INTO STUD VALUES(3, '장길산');
COMMIT;

INSERT INTO LECTURE_STUDENT VALUES(1, 1);
INSERT INTO LECTURE_STUDENT VALUES(1, 2);
INSERT INTO LECTURE_STUDENT VALUES(1, 3);

INSERT INTO LECTURE_STUDENT VALUES(2, 1);
COMMIT;
...
CREATE TABLE EMP_UK(
	...
	ENMAE VARCHAR2(20) CONSTRAINT UK_EMP_UK_ENAME UNIQUE
);
...
-- NOT NULL 제약조건이 지정된 칼럼의 데이터로 NULL 저장하면 에러가 발생한다.
...
-- 1-6. DEFAULT: 특정 컬럼의 데이터가 NULL로 오거나 들어오지 않았을 때 대비해서 기본값을 지정해놓은 제약조건
-- DBMS마다 DEFAULT와 NOT NULL의 순서 차이가 있지만 기본적으로 DEFUALT 제약조건이 먼저 설정돼야 하는 DBMS가 대부분이다.
...
-- LOC(VARCHAR2(10), DEFAULT '서울', NOT NULL) 컬럼을 갖는 FACTORY1 생성하세요
...
-- FNO(NUMBER(10), FK(FACTORY1의 FNO), NOT NULL) 컬럼을 갖는 GOODS1 테이블 생성하세요
CREATE TABLE GOODS1(
...
-- PRICE(NUMBER(5), DEFAULT 7000), PDATE(DATE, DEFAULT SYSTATE, NOT NULL)
